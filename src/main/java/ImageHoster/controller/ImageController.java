...
public class ImageController {
    ...
    //Solution is:This method is called when the details of the specific image with corresponding imageId and title are to be displayed
    ...
    //First receive the dynamic parameter in the incoming request URL in a Integer variable 'imageId' in a string variable 'title' and also the Model type object
    //Call the getImage() method in the business logic to fetch all the details of that image according to its imageId
    //Add the image in the Model type object with 'image' as the key
    //Return 'images/image.html' file

    //Also now you need to add the comments of an image in the Model type object
    ...
    //The method get comments add this in a Model type object
    //This string is then displayed by 'edit.html' file as previous comments of an image

    //If the owner of the image is trying to edit the image, the user must be redirected to 'images/edit'.
    // But if the non-owner of the image is trying to edit the image, return the ‘images/image.html’ file displaying all the details of the image and print the message ‘Only the owner of the image can edit the image'.
    //The Authentication is done using authenticateUser()
    @RequestMapping(value = "/editImage")
    public String editImage(@RequestParam("imageId") Integer imageId, Model model,HttpSession session) {
        Image image = imageService.getImage(imageId);
        boolean authenticateValue=authenticateUser(imageId,session);
    ...
    //This method gets list of comments using  getAllComments() and later on set comments received by getAllComments()
    @RequestMapping(value = "/editImage", method = RequestMethod.PUT)
    public String editImageSubmit(@RequestParam("file") MultipartFile file, @RequestParam("imageId") Integer imageId, @RequestParam("tags") String tags, Image updatedImage,Comment comment, HttpSession session) throws IOException {
        Image image = imageService.getImage(imageId);
        ...
        updatedImage.setComments(comments);
    ...
    //If the owner of the image is trying to delete the image, the image is deleted and the user must be redirected to '/images'.
    ...
    //The method receives the imageId and session
    //It authenticates ids of the current user and image user
    //Returns the boolean
    public boolean authenticateUser(Integer imageId,HttpSession session){
        Image image = imageService.getImage(imageId);
        User currentUser=(User)session.getAttribute("loggeduser");
        Integer currentUserId=currentUser.getId();
        User imageUser=image.getUser();
        Integer imageUserId=imageUser.getId();
